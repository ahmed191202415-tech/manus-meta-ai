...
async def view_dashboard(dashboard_id: str):
    ...
    runtime_queries = (
        ...
        or (data_contract.get("runtime_queries") if isinstance(data_contract, dict) else None)
        or (data_contract.get("queries") if isinstance(data_contract, dict) else None)
    )
    if runtime_queries:
        # Runtime-backed dynamic dashboards share the same live renderer as
        # manifest dashboards created through the ChatGPT definition API.
        # This prevents a saved backend contract from being rendered as a
        # static snapshot-only page.
        from app.api.journey_dashboard_v7 import _custom_dashboard_html

        definition = DashboardStore.definition_from_manifest(row)
