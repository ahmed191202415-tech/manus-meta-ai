...
async def list_page_posts(
    ...
    if after:
        params["after"] = after
    if fetch_all:
        return meta_get_all_pages(f"{page_id}/posts", token, params=params, max_pages=max_pages)
    return meta_call("GET", f"{page_id}/posts", token, params=params)
...
async def create_page_post(body: PagePostCreateRequest, token: str = Depends(resolve_access_token)):
    ...
    return meta_call("POST", f"{body.page_id}/feed", token, data=payload)


@router.delete("/page_posts/{post_id}")
async def delete_page_post(post_id: str, token: str = Depends(resolve_access_token)):
    return meta_call("DELETE", post_id, token)
...
async def list_page_comments(
    ...
    if after:
        params["after"] = after
    if fetch_all:
        return meta_get_all_pages(f"{object_id}/comments", token, params=params, max_pages=max_pages)
    return meta_call("GET", f"{object_id}/comments", token, params=params)


@router.post("/page_comments/{comment_id}/reply")
async def reply_to_comment(comment_id: str, body: CommentReplyRequest, token: str = Depends(resolve_access_token)):
    payload: Dict[str, Any] = {"message": body.message}
    payload.update(body.extra_params)
    return meta_call("POST", f"{comment_id}/comments", token, data=payload)


@router.post("/page_comments/{comment_id}/hide")
async def hide_comment(comment_id: str, body: CommentHideRequest, token: str = Depends(resolve_access_token)):
    payload = {"is_hidden": body.is_hidden}
    return meta_call("POST", comment_id, token, data=payload)


@router.delete("/page_comments/{comment_id}")
async def delete_comment(comment_id: str, token: str = Depends(resolve_access_token)):
    return meta_call("DELETE", comment_id, token)


@router.post("/objects/{object_id}/delete")
async def delete_object_raw(object_id: str, body: ObjectDeleteRequest, token: str = Depends(resolve_access_token)):
    payload = dict(body.extra_params or {})
    payload["method"] = "delete"
    return meta_call("POST", object_id, token, data=payload)
...
async def page_insights(
    ...
    if until:
        params["until"] = until
    return meta_call("GET", f"{page_id}/insights", token, params=params)
