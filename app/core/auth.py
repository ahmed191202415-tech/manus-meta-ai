...
def _resolve_default_saved_connection(request: Request) -> str | None:
    ...
    Uses the newest active tenant with a saved Meta connection. This restores the
    old behavior where the GPT could work from the saved Meta connection without
    requiring the user to re-authenticate inside ChatGPT.
    """
    try:
