...
def _missing_required_input(node: DashboardQueryNode, key: str, inputs: dict) -> bool:
    ...
    if key in {"campaign_id", "adset_id", "ad_id"} and not _present_input(lookup(inputs, key)):
        # Entity-list operations power strict cascading controls. Their parent
        # selector is genuinely required, while analytical queries may still
        # treat an ``all`` child selector as an optional scope refinement.
        if node.connector == "meta" and node.operation in {"list_adsets", "list_ads"}:
            return True
        return False
...
async def execute_plan(plan: DashboardQueryPlan, request: Request, inputs: dict | None = None, trigger: str = "manual") -> dict:
    ...
    for node_id in validation["execution_order"]:
        ...
        missing_inputs = [key for key in node.required_inputs if _missing_required_input(node, key, inputs)]
        if missing_inputs:
