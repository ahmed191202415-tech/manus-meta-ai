...
def test_runtime_manifest_uses_live_renderer_instead_of_static_snapshot(monkeypatch):
    row = _dashboard_row()
    row["config"].update(
        {
            "filters": [
                {"key": "account_id", "type": "select", "options_source": {"resource": "accounts"}},
                {"key": "campaign_id", "type": "select", "depends_on": "account_id", "options_source": {"resource": "campaigns"}},
            ],
            "widgets": [{"id": "funnel", "title": "Funnel", "type": "funnel", "data_query": "journey_funnel"}],
            "data_contract": {
                "runtime_queries": {
                    "accounts": {"connector": "meta", "resource": "accounts"},
                    "campaigns": {"connector": "meta", "resource": "campaigns", "depends_on": "account_id"},
                    "journey_funnel": {"connector": "journey", "resource": "funnel"},
                }
            },
        }
    )
    ...
    response = asyncio.run(dynamic_dashboards.view_dashboard("dash_1"))
